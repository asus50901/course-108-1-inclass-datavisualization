...
```{r check your bbox and crs}
sf_mrtStops_tpe_BL %>% st_bbox # check the boundaries of the bounding box
sf_mrtStops_tpe_BL %>% st_crs # check the CRS of your data
```
...
```{r load and shrink your data size}
...
```{r check your bbox and crs again}
...
```{r extract the CRS data and the station positions}
...
```{r check the structure of CRS and station pos data}
...
```{r dot the stations on north taiwan}
graph$northTaiwan +
  geom_sf(
...
```{r link the station points to form a route}
...
# Only sf and sfc class can be drawn on a plot

# st_geometry() has 2 usage, 1. extract the pin-point coords from a sfc class
  # 2. Transform an object into a sfc object
...
sfc_BL_line %>% 
  st_set_crs(
    ...
    # st_set_crs() applies when the original object doesn't possess a crs
    # st_transform() applies to object that originally have a CRS
  ) -> sfc_BL_line
...
```{r check all the CRs are in harmony}
...
```{r zoom by qualifing the coords}
graph$northTaiwan_with_BL+
  coord_sf(
    xlim=c(121.4,121.7),
    ylim=c(24.9,25.09)
